...
import { AttachFileOutlined, LockOutlined, SendOutlined } from "@mui/icons-material";
import { Avatar } from "./Avatar";
import { poidsLisible } from "./ApercuFichier";
...
export function ZoneCommentaire({
  ...
  busy: boolean;
  libelleEnvoi?: string;
  /** Propose « Note interne » : un message que seule l'équipe lira. Absent = jamais proposé. */
  ...
  async function envoyer() {
    ...
    await onEnvoyer(corps, fichier, retenues, optionInterne && interne);
    setTexte("");
    setInterne(false);
    setFichier(null);
    setDesignees([]);
  ...
  return (
    <div>
      ...
      <div className="mt-2 flex items-center gap-2">
        ...
        {optionInterne && (
          <label
            className={`inline-flex items-center gap-1.5 h-8 px-2.5 rounded-lg border text-body-sm cursor-pointer transition-colors ${
              interne
                ? "border-primary bg-primary/10 text-primary"
                : "border-outline-soft text-on-surface-variant hover:bg-surface-container-low"
            }`}
            title="Un message interne n'est jamais visible par le client"
          >
            <input
              type="checkbox"
              className="sr-only"
