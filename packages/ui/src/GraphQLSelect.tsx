...
import { useGraphQLRecords } from "./hooks/useGraphQLRecords";

interface GraphQLSelectProps<T> {
  app: string;
  model: string;
  searchFields?: string[];
  include?: string[];
...
export function GraphQLSelect<T = Record<string, unknown>>({
  app,
  model,
  searchFields,
  include,
  ...
  const { items, loading, query, setQuery } = useGraphQLRecords<T>({
    app,
    model,
    searchFields,
    include,
