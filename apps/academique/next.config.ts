...
// L'app tourne derrière nginx sur son domaine public (ACADEMIQUE_APP_URL).
const allowedDevOrigins = ["127.0.0.1", "localhost"];
if (process.env.ACADEMIQUE_APP_URL) {
  try {
    allowedDevOrigins.push(new URL(process.env.ACADEMIQUE_APP_URL).host);
  } catch {}
}
