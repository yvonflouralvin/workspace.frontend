...
frontends/
  apps/
    ...
    comptabilite/ → port 3020  (comptabilité générale OHADA — écritures, grand livre, états)
    business-firm-missions/ → port 3021  (suivi missions/prestations Business Firm — clients, missions, audit, fiscal)
...
| `comptabilite` | 3020 | **Comptabilité générale OHADA** (backend `comptabilite`, port 5000) — plan comptable (référentiel OHADA chargeable en un clic), journaux, exercices, écritures (brouillon → validée → contre-passée, équilibre débit/crédit imposé), lettrage manuel, grand livre, balance, bilan/compte de résultat simplifiés (groupés par classe OHADA), taxes, comptabilisation automatique d'une facture `ventes` (ventilation HT/TVA/TTC). Verrouillage au niveau exercice uniquement (pas de période mensuelle), un compte de contrepartie unique par classe de tiers (pas de compte auxiliaire par tiers) — coupes de portée assumées pour la V1. | — |
| `business-firm-missions` | 3021 | **Business Firm Mission** (backend `business_firm_missions`, port 5000) — surface unifiée pour le suivi des missions et prestations d'un cabinet pour ses clients : **Clients** (lit/écrit directement `tiers`, étendu de secteur d'activité et de contrats — aucun registre client dupliqué. **Une mission n'est visible que de son responsable, de son créateur et de son équipe** (permission `business_firm_missions.missions.view_all` pour tout voir — groupe Direction, propriétaire ; `manage` ne suffit pas) ; on n'assigne une tâche qu'à un membre de la mission. **Chaque collaborateur reçoit, à son ajout, une liste de droits** (modifier la mission, ajouter/modifier une phase, ajouter/modifier une tâche, valider les résultats) ; sans droit, il lit et travaille sur les tâches qui lui sont assignées. **Cycle d'une tâche** : À faire → En cours → Terminée (par l'exécutant) → Validée (par le validateur que le responsable désigne à la création), ou À revoir ; c'est le serveur qui dit ce que chacun peut faire (`statuts_possibles`, `mes_droits`), les écrans ne le redéduisent pas. La fiche client a pour onglets Informations générales (seul endroit où le client se modifie ; l'en-tête, lui, est en lecture seule : nom, e-mail, ville), Contacts, Contrats, Missions, Factures. Tout ajout ou modification s'y fait dans un `FormDrawer`, jamais dans la liste — vrai aussi des sept onglets d'un dossier d'**Assistance fiscale**. La suppression d'un contact demande confirmation). **Type de tâche** choisi à la création (Tâche · Document à fournir · Contrôle à effectuer) : un **contrôle** porte, dans l'aperçu de la tâche elle-même (pas d'onglet à part), son observation, ses anomalies, risques, recommandations et **actions correctives** (planifiée → exécutée → résultat validé, avec qui a exécuté et qui a validé) ; ses documents sont des pièces justificatives. Un onglet **Historique** (toute tâche) dit qui a fait quoi, qui a changé quel statut, qui a validé quoi ; la **mission** a le sien, qui réunit le journal de toutes ses tâches (chaque ligne située par sa tâche) — et une tâche ou une phase supprimée y reste, avec la personne qui l'a supprimée. **Les listes de tâches se lisent, elles ne se modifient pas** : un clic sur une ligne (mission, phase, « Mes tâches ») ouvre le `TacheDrawer`, seul endroit où l'on édite, **Mes tâches** (`/taches`, entrée « Tâches » du menu : les tâches assignées à l'utilisateur connecté, toutes missions confondues — regroupées par échéance (en retard, aujourd'hui, à venir, sans échéance), terminées masquées par défaut ; l'utilisateur vient de la session, jamais d'un paramètre ; endpoint `GET /bfm/mes-taches` ; la tâche assignée « au client » n'y figure pas), **Missions** (modèle dédié du service `business_firm_missions` — missions, phases, tâches, équipe —, détaché de `projects`. La fiche mission est en **routes** sur le modèle de Projets : `/missions/[id]` (aperçu), `/phases`, `/phases/[phaseId]` (page de la phase, ses propres onglets), `/parametres` puis `/parametres/collaborateurs`. Onglets à icônes, dont **Tâches** et **Documents** au niveau de la mission (toutes phases confondues). Les tâches s'y voient en **liste** (regroupée par phase) ou en **kanban** (glisser une carte change son statut), avec les filtres « assignées à moi » et « assignées au client » — un seul composant (`VueTaches`) sert la mission et chaque phase. Le tiroir d'une phase et celui d'une tâche offrent « Ouvrir la page ». La **page d'une tâche** (`/phases/[phaseId]/taches/[tacheId]`) a un onglet Aperçu (description riche, fil de discussion avec réponses, mentions, pièces jointes et **notes internes**) et un onglet **Documents** qui recense tous les fichiers de la tâche, joints à un commentaire ou déposés directement ; un document se **renomme** en place et a sa **propre discussion** (tiroir, compteur de messages). Une tâche peut être **assignée au client** de la mission. Tout choix de personne passe par `components/SelecteurMembre` — popup `PaletteRecherche` sur les membres du workspace —, jamais par un ID saisi à la main). **Portail client** (`/portail/*`) : un contact d'une entreprise cliente se connecte (compte donné depuis la fiche client, onglet Contacts) et voit les missions de SON client, les tâches qui lui sont assignées, leur fil (jamais les notes internes) et leurs documents. C'est un membre du groupe « Clients — Business Firm Mission » — créé par `auth`, qui ne porte que `business_firm_missions.portail` et dont la page de démarrage est l'app — et il n'ouvre aucun écran interne (`DashboardShell` le renvoie à `/portail`). Les types du portail (`lib/bfm-portail-api.ts`) sont séparés de ceux de l'équipe : ni budget, ni heures, **Audit** (domaine neuf et indépendant du service `audit_missions`, qui porte une offre distincte vendue aux clients — missions, équipe, planning, documents demandés/reçus/manquants, checklist à 5 statuts, risques distincts des anomalies, rapports provisoire/final, historique), **Assistance fiscale** (domaine neuf — dossiers, obligations, échéances avec alerte sur l'accueil du module, déclarations, correspondances, paiements, pénalités, contrôles fiscaux). | — |
...
| `web`       | —    | Site vitrine public — présente l'entreprise et la vitrine de chaque module (`lib/produits.ts`, page dynamique `/produits/[slug]`) | —  |
...
`ConfirmDialog` / `Toast` (**plus aucun `confirm()` ni `alert()` natif**) — et **toute suppression passe par une confirmation**, via `useConfirmSuppression` (`@repo/ui/hooks/useConfirmSuppression`), `SearchField`,
