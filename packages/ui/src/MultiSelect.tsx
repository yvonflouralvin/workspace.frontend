...
export function MultiSelect({
  ...
  placeholder?: string;
  emptyLabel?: string;
  // En lecture seule, les tags restent LISIBLES : les masquer ferait croire
  // qu'aucun n'est sélectionné.
  disabled?: boolean;
}) {
  ...
  return (
    <div ref={containerRef} className="relative">
      <div
        className={`flex flex-wrap items-center gap-1.5 p-2 rounded-xl border border-outline-variant bg-surface min-h-[2.75rem] ${
          disabled ? "opacity-70" : ""
        }`}
      >
        {selected.map((option) => (
        ...
        <div className="flex items-center flex-1 min-w-[8rem] gap-1">
          ...
          <input
            ...
            className="w-full px-1 py-1 text-sm text-on-surface bg-transparent outline-none disabled:cursor-default"
          />
        </div>
      </div>
