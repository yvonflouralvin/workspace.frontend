...
export const metadata: Metadata = {
  title: "Comptabilité",
  description: "Comptabilité générale OHADA",
};

export default async function RootLayout({ children }: { children: React.ReactNode }) {
  const session = await exigerSession();
  const accessDenied =
    session.authenticated && !session.permissions.includes("comptabilite.access");
  const canSwitchTo = session.workspaces.some(
    (ws) => ws.id !== session.active_workspace?.id && ws.permissions.includes("comptabilite.access")
  );
        ...
        <SessionProvider initialSession={session}>
          {accessDenied ? (
            <AccessDenied
              ...
              switcher={
                canSwitchTo ? (
                  <WorkspaceSwitcher filterPermission="comptabilite.access" />
                ) : undefined
              }
            />
          ) : (
