...
export const metadata: Metadata = {
  title: "SGR",
  description: "Secrétariat Général à la Recherche",
};
...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const accessDenied =
    session.authenticated && !session.permissions.includes("sgr.access");
  const canSwitchTo = session.workspaces.some(
    (ws) => ws.id !== session.active_workspace?.id && ws.permissions.includes("sgr.access")
  );
        ...
        <SessionProvider initialSession={session}>
          {accessDenied ? (
            <AccessDenied
              ...
              switcher={
                canSwitchTo ? (
                  <WorkspaceSwitcher filterPermission="sgr.access" />
                ) : undefined
              }
            />
          ) : (
