...
// L'app tourne derrière nginx sur son domaine public (APPROVAL_FLOWS_APP_URL).
const allowedDevOrigins = ["127.0.0.1", "localhost"];
if (process.env.APPROVAL_FLOWS_APP_URL) {
  try {
    allowedDevOrigins.push(new URL(process.env.APPROVAL_FLOWS_APP_URL).host);
  } catch {}
}
