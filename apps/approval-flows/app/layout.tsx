...
export default async function RootLayout({
  ...
  const accessDenied =
    session.authenticated && !session.permissions.includes("approval_flows.access");
  const canSwitchTo = session.workspaces.some(
    (ws) => ws.id !== session.active_workspace?.id && ws.permissions.includes("approval_flows.access")
  );
        ...
        <SessionProvider initialSession={session}>
          {accessDenied ? (
            <AccessDenied
              ...
              switcher={canSwitchTo ? <WorkspaceSwitcher filterPermission="approval_flows.access" /> : undefined}
            />
          ) : (
            children
          )}
