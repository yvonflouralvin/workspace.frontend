...
// L'app tourne derrière nginx sur son domaine public (TIERS_APP_URL).
const allowedDevOrigins = ["127.0.0.1", "localhost"];
if (process.env.TIERS_APP_URL) {
  try {
    allowedDevOrigins.push(new URL(process.env.TIERS_APP_URL).host);
  } catch {}
}
