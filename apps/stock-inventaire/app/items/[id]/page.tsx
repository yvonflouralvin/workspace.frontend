...
export default function ItemDetailPage() {
  ...
  const [mouvForm, setMouvForm] = useState<{ type_mouvement: TypeMouvement; quantite: string; reference: string; notes: string }>({
  ...
  useEffect(() => {
    Promise.all([
      ...
      setItem(itemData);
      setCategories(cats);
    }).catch(() => setError("Article introuvable")).finally(() => setLoading(false));
  }, [id]);
  ...
  if (loading) {
    return <DashboardShell><div className="p-8 text-body-md text-on-surface-variant">Chargement…</div></DashboardShell>;
  }
  if (!item) {
    return <DashboardShell><div className="p-8 text-body-md text-error">{error}</div></DashboardShell>;
  }

  const stockAlert = item.gestion_stock && item.stock_minimum != null && item.stock_actuel <= item.stock_minimum;
      ...
      <div className="p-8 max-w-2xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {!item.is_active && (
                  <span className="px-2 py-0.5 rounded-full text-label-md font-medium bg-error/10 text-error">Inactif</span>
                )}
              </div>
              <p className="text-body-sm text-on-surface-variant font-mono mt-0.5">{item.code}</p>
            </div>
          </div>
          {!editing && canManage && item.is_active && (
        ...
        {confirmDeactivate && (
          <div className="bg-error-container/30 border border-error/20 rounded-xl p-4 space-y-3">
            ...
            <p className="text-body-sm text-on-surface-variant">L&apos;article sera masqué du catalogue mais les données seront conservées.</p>
        ...
        {!editing ? (
          <div className="space-y-6">
            <div className="bg-surface-container-lowest border border-outline-variant rounded-xl p-6 space-y-6">
              <div>
                <h2 className="text-body-md font-semibold text-on-surface border-b border-outline-variant pb-2 mb-4">Identification</h2>
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <ReadField label="Code" value={<span className="font-mono">{item.code}</span>} />
                </div>
                {item.description && (
                  <div className="mt-4">
                    <ReadField label="Description" value={item.description} />
                  </div>
                )}
              </div>

              <div>
                <h2 className="text-body-md font-semibold text-on-surface border-b border-outline-variant pb-2 mb-4">Stock</h2>
                {item.gestion_stock ? (
                  <div className="grid grid-cols-2 gap-4">
                    <div className="flex flex-col gap-0.5">
                    ...
                    <ReadField label="Seuil d'alerte" value={item.stock_minimum != null ? `${Number(item.stock_minimum)} ${item.unite}` : undefined} />
                  </div>
                ) : (
              ...
              <div>
                <h2 className="text-body-md font-semibold text-on-surface border-b border-outline-variant pb-2 mb-4">Vente</h2>
                {item.est_vendu ? (
                  <div className="grid grid-cols-2 gap-4">
                    <ReadField label="Prix de vente" value={item.prix_vente != null ? `${Number(item.prix_vente).toLocaleString("fr-CD")} CDF` : undefined} />
                    <ReadField label="Taux TVA" value={item.tva_taux != null ? `${Number(item.tva_taux)}%` : undefined} />
                  </div>
                ) : (
              ...
              {item.notes && (
                <div>
                  <h2 className="text-body-md font-semibold text-on-surface border-b border-outline-variant pb-2 mb-4">Notes</h2>
                  <p className="text-body-md text-on-surface whitespace-pre-line">{item.notes}</p>
                </div>
              )}
            </div>

            {/* Mouvements de stock */}
            {item.gestion_stock && canViewMouvements && (
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <h2 className="text-body-md font-semibold text-on-surface">Mouvements de stock</h2>
                  {canMouvements && (
                    <button
                ...
                {showMouvModal && (
                  <form onSubmit={handleMouvement} className="bg-surface-container-lowest border border-outline-variant rounded-xl p-4 space-y-3">
          ...
          /* Edit mode */
          <form onSubmit={handleSave} className="space-y-6 bg-surface-container-lowest border border-outline-variant rounded-xl p-6">
            ...
            <div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    <label className="text-label-md text-on-surface-variant">Type *</label>
                    <select value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value as TypeItem }))} className={inputCls}>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <input type="text" value={form.nom as string} onChange={(e) => setForm((f) => ({ ...f, nom: e.target.value }))} className={inputCls} required />
                </div>
                <div className="flex flex-col gap-1">
                  <label className="text-label-md text-on-surface-variant">Description</label>
                  <textarea value={form.description as string} onChange={(e) => setForm((f) => ({ ...f, description: e.target.value }))} rows={2} className={`${inputCls} resize-none`} />
                </div>
                <div className="grid grid-cols-2 gap-4">
                  <div className="flex flex-col gap-1">
                    <label className="text-label-md text-on-surface-variant">Référence / SKU</label>
                    <input type="text" value={form.reference as string} onChange={(e) => setForm((f) => ({ ...f, reference: e.target.value }))} className={inputCls} />
                  </div>
                  <div className="flex flex-col gap-1">
                    <label className="text-label-md text-on-surface-variant">Unité</label>
                    <select value={form.unite as string} onChange={(e) => setForm((f) => ({ ...f, unite: e.target.value }))} className={inputCls}>
            ...
            <div>
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={!!form.gestion_stock} onChange={(e) => setForm((f) => ({ ...f, gestion_stock: e.target.checked }))} className="h-4 w-4 rounded border-outline-variant text-primary" />
                ...
                {form.gestion_stock && (
                  <div className="flex flex-col gap-1 pl-7">
                    ...
                    <input
                      ...
                      onChange={(e) => setForm((f) => ({ ...f, stock_minimum: e.target.value ? Number(e.target.value) : null }))}
                      placeholder="Optionnel"
                      className={inputCls}
                    />
                  </div>
            ...
            <div>
              ...
              <div className="space-y-3">
                <label className="flex items-center gap-3 cursor-pointer">
                  <input type="checkbox" checked={!!form.est_vendu} onChange={(e) => setForm((f) => ({ ...f, est_vendu: e.target.checked }))} className="h-4 w-4 rounded border-outline-variant text-primary" />
                ...
                {form.est_vendu && (
                  <div className="grid grid-cols-2 gap-4 pl-7">
                    <div className="flex flex-col gap-1">
                      ...
                      <input type="number" min="0" step="0.01" value={form.prix_vente ?? ""} onChange={(e) => setForm((f) => ({ ...f, prix_vente: e.target.value ? Number(e.target.value) : null }))} className={inputCls} />
                    </div>
                    <div className="flex flex-col gap-1">
                      <label className="text-label-md text-on-surface-variant">Taux TVA (%)</label>
                      <input type="number" min="0" max="100" step="0.01" value={form.tva_taux ?? ""} onChange={(e) => setForm((f) => ({ ...f, tva_taux: e.target.value ? Number(e.target.value) : null }))} placeholder="Optionnel" className={inputCls} />
                    </div>
                  </div>
                )}
              </div>
            ...
            <div className="flex flex-col gap-1">
              ...
              <textarea value={form.notes as string} onChange={(e) => setForm((f) => ({ ...f, notes: e.target.value }))} rows={3} className={`${inputCls} resize-none`} />
            </div>

            <div className="flex items-center justify-end gap-3">
              <button type="button" onClick={() => { setEditing(false); setError(null); }} className="px-4 py-2 rounded-xl text-body-md text-on-surface-variant border border-outline-variant hover:bg-surface-container transition-colors">
                Annuler
              </button>
              <button type="submit" disabled={saving} className="px-5 py-2 rounded-xl bg-primary text-on-primary text-body-md font-medium hover:bg-primary-container disabled:opacity-60 transition-colors">
