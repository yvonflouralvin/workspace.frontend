...
export default async function RootLayout({ children }: { children: React.ReactNode }) {
  ...
  const accessDenied =
    session.authenticated && !session.permissions.includes("operations.access");
  const canSwitchTo = session.workspaces.some(
    (ws) => ws.id !== session.active_workspace?.id && ws.permissions.includes("operations.access")
  );
        ...
        <SessionProvider initialSession={session}>
          {accessDenied ? (
            <AccessDenied
              ...
              switcher={
                canSwitchTo ? (
                  <WorkspaceSwitcher filterPermission="operations.access" />
                ) : undefined
              }
            />
          ) : (
